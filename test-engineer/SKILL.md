---
name: test-engineer
description: 测试工程师 - 版本测试、需求管理、测试用例生成、测试方案设计、测试报告、性能基线、归档、测试数据管理、测试专家咨询、知识回顾。TRIGGER when: 任何测试相关请求（"开始测试""写测试方案""生成用例""测试报告""添加需求""测试进度""审一下用例""怎么测""给点建议""分析缺陷""版本更新说明""基线""归档""复用""回顾""知识库健康""进化日志"）。
---
...
version_patterns:
  ...
  平台线: "v1\\.\\d+|MP_v[\\d.]+"
  APP: "V7\\.\\d+\\.\\d+"
modules: [req-manager, test-case-generator, test-data-manager, algo-test-report, platform-test-report, release-notes, performance-baseline, archive-manager, evolution]
paths:
  test_expert: "../sweeper-robot-test/SKILL.md"
  knowledge_base: "../shared-knowledge/knowledge-base/"
  product_knowledge: "../shared-knowledge/platform-app-product-knowledge.md"
```
...
**版本号格式**（从配置的 version_patterns 读取，默认值见内置配置）：
- 算法线：A2.xx
- 平台线：v1.xx / MP_vX.X.X
- APP：V7.x.x
...
| 触发词 | 动作 |
|---|---|
| "写算法测试报告" | → `Read modules/algo-test-report.md` |
| "写平台测试报告" | → `Read modules/platform-test-report.md` |
...
| 话题 | 额外加载 |
|---|---|
| 审用例/测试点 | + `{paths.product_knowledge}`（默认 `../shared-knowledge/platform-app-product-knowledge.md`） |
| 设计测试方案 | + `{paths.knowledge_base}business-rules/` 相关文件 |
| 分析缺陷/问题定位 | + `{paths.knowledge_base}defect-patterns/` + `system-baselines/` |
| 一般咨询 | 仅 test-expert |
...
2. **沉淀检查** — 有无新的测试经验或缺陷模式值得沉淀到 `{paths.knowledge_base}`（默认 `../shared-knowledge/knowledge-base/`）
