...
- **对比报告和趋势报告** → 输出到 `{output.root}/_知识库/性能基线/reports/`
- **报告署名**：周健荣
...
5. 退步项做根因分析提示
6. 生成对比报告(MD)到 {output.root}/_知识库/性能基线/reports/
7. 输出摘要给用户
```
...
输出到：`{output.root}/_知识库/性能基线/reports/基线管理报告-{产品}-{日期}.md`
...
SKILL.md（调度）
    ...
    └─ 执行完成后（后期验证）
        ...
        performance-baseline（接口3：基线对比）
            ...
            ├── 生成对比报告 → 输出到 {output.root}/_知识库/性能基线/reports/
